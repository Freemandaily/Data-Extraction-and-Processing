    ...
    try:
        if not timeframe_prices.token_interval_prices:
            ...
            if response == 200:
                ...
                st.error('Requesting Data from Moralis Failed! Please Reload The Page An Try Again')
                st.stop()
        else:
...
def dexScreener_token_data(mint_address): # fetches token pairs seaching with token address (2)
    ...
    try:
        ...
        st.error(f'Error :Failed to connect to {url}! Please Reload The Page')
        st.stop()
       #  print(f'Error :Failed to connect to {url}')
    except requests.exceptions.Timeout:
        st.error(f"Error: Request timed out for{url}!. Please Reload The Page")
        st.stop()
        # print(f"Error: Request timed out for{url}")
    except requests.exceptions.RequestException as e:
        st.error(f"Error: A network-related error occurred! Please Reload The Page:")
        st.stop()
